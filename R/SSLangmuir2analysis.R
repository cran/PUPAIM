#' @title Langmuir Isotherm Nonlinear Analysis via selfStart and Langmuir Second Linear Model
#' @name SSLangmuir2analysis
...
#' @examples SSLangmuir2analysis(Ce,Qe)
...
SSLangmuir2analysis <- function(Ce, Qe){
  ...
  parsinit2 <- getInitial(Qe ~ SSLangmuir2(Ce,Qmax,Kl), data = data2)

  ### Nonlinear fitting using selfStart function
  fit1 <- nls(Qe ~ SSLangmuir2(Ce,Qmax,Kl), data = data2 )

  ### Predicted paramaters from nonlinear fitting
  parsLangmuir2 <- as.vector(coefficients(fit1))
  pars_Qmax <- parsLangmuir2[1L];
  pars_Kl <- parsLangmuir2[2L];
