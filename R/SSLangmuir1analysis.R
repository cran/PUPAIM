#' @title Langmuir Isotherm Nonlinear Analysis via selfStart and Langmuir First Linear Model
#' @name SSLangmuir1analysis
...
#' based on Langmuir first linear model, predicted parameter values, and
#' model error analysis
#' @examples Ce <- c(0.01353, 0.04648, 0.13239, 0.27714, 0.41600, 0.63607, 0.80435, 1.10327, 1.58223)
#' @examples Qe <- c(0.03409, 0.06025, 0.10622, 0.12842, 0.15299, 0.15379, 0.15735, 0.15735, 0.16607)
#' @examples SSLangmuir1analysis(Ce,Qe)
...
SSLangmuir1analysis <- function(Ce, Qe){

  x <- Ce
  y <- Qe
  data1 <- data.frame(x, y)
  data2 <- data.frame(data1, end = 1:nrow(data1))

  ### Initial starting values of the selfStart function

  suppressWarnings(parsinit1 <- getInitial(Qe ~ SSLangmuir1(Ce,Qmax,Kl), data = data2))

  ### Nonlinear fitting using selfStart function
  suppressWarnings(fit1 <- nls(Qe ~ SSLangmuir1(Ce,Qmax,Kl), data = data2 ))

  ### Predicted paramaters from nonlinear fitting
  parsLangmuir1 <- as.vector(coefficients(fit1))
  pars_Qmax <- parsLangmuir1[1L];
  pars_Kl <- parsLangmuir1[2L];
...
# Error analysis of the Langmuir isotherm model
  ...
  ggplot2::ggplot(data1, aes(x, y)) + ggplot2::geom_point(color ="#3498DB" ) +
    ...
    ggplot2::labs(x = "Ce",
       ...
       subtitle = "via selfStart and first linear form",
